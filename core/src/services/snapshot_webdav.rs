...
pub async fn try_upload_snapshot(project_dir: &str, meta: &SnapshotMeta) {
        ...
        .filter_map(|e| e.ok())
    {
        ...
        if path.is_dir() {
            ...
            if let Err(e) = upload_file(&ctx.client, &ctx.base_url, &remote_item, path, &ctx.auth).await {
    ...
    if let Err(e) =
        upload_file(&ctx.client, &ctx.base_url, &remote_meta, &local_meta, &ctx.auth).await
    {
